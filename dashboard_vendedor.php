...
// Vendas realizadas
$sql_vendas = "SELECT COUNT(*) AS total_vendas FROM vendas WHERE id_vendedor = ?";
...
// Faturamento total
$sql_faturamento = "SELECT SUM(valor_total) AS faturamento FROM vendas WHERE id_vendedor = ?";
...
// Faturamento mensal
$sql_faturamento_mensal = "SELECT MONTH(data_venda) AS mes, SUM(valor_total) AS total FROM vendas WHERE id_vendedor = ? GROUP BY mes ORDER BY mes";
...
// Últimos produtos
$sql_ultimos = "SELECT nome, preco, estoque, imagem, data_cadastro FROM produtos WHERE id_vendedor = ? ORDER BY data_cadastro DESC LIMIT 5";
